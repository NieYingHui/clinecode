...
from clinecode.skills.parser import SkillDef, substitute_arguments
from clinecode.tools import ToolRegistry

if TYPE_CHECKING:
    from clinecode.agent import Agent, AgentEvent
...
class SkillExecutor: # Skill执行
    ...
    async def execute_fork(
        ...
        fork_conv = ConversationManager() #创建全新的ConversationManager


        context_messages = self._build_fork_context(skill.context) #，先注入上下文消息
        ...
        fork_conv.add_user_message(prompt) #再把 Skill prompt 作为最后一条用户消息加入

        try:
            filtered_registry = filter_tool_registry( #接着创建过滤后的工具注册中心
                self.agent.registry, skill.allowed_tools
            )
        except SkillDependencyError as e:
            return f"Skill execution failed: {e}"

        from clinecode.agent import Agent as AgentClass, StreamText, LoopComplete, ErrorEvent

        fork_agent = AgentClass( #独立子 Agent
            client=self.client,
