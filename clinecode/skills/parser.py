...
@dataclass
class SkillDef: # SkilMeta：Skill 的元信息
    name: str
    description: str
    prompt_body: str = ""
    allowed_tools: list[str] = field(default_factory=list) #双重作用：控制副作用范围（子Agent 只能用这些工具），也控制信息可见性（看不到的工具等于不存在）
...
def parse_frontmatter(raw: str) -> tuple[dict, str]:
    ...
    try:
        meta = yaml.safe_load(yaml_block) #用yaml.safe_load而不是yaml.load，因为yaml.load允许在YAML 里嵌入任意对象（包括执行代码）
