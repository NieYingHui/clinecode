...
from clinecode.worktree.changes import (
    CleanupResult,
    Changes,
    count_worktree_changes,
    has_worktree_changes,
)
from clinecode.worktree.models import Worktree, WorktreeSession
from clinecode.worktree.session import load_worktree_session, save_worktree_session
from clinecode.worktree.setup import perform_post_creation_setup
from clinecode.worktree.slug import flatten_slug, validate_slug
...
class WorktreeManager:
    def __init__(
        ...
        self.repo_root = repo_root # 主仓库路径
        self.symlink_directories = symlink_directories or []
        self.worktree_dir = worktree_dir or str( # Worktree存放目录
            Path(repo_root) / ".clinecode" / "worktrees"
        )
        self._clinecode_dir = Path(repo_root) / ".clinecode"
        self._lock = asyncio.Lock() # 并发保护
        self.active: dict[str, Worktree] = {} # name -> Worktree
        self.current_session: WorktreeSession | None = None # 当前活跃的worktree会话
    ...
    # ·git 文件→gitdir：指针→HEAD→ref解析→loose ref或packed-refs→SHA。
    @staticmethod
    ...
    async def create(self, name: str, base_branch: str = "HEAD") -> Worktree:
        async with self._lock:
            err = validate_slug(name) # 1.验证名称
            if err:
                raise WorktreeError(err)

            if name in self.active: # 2.锁内检查是否已存在
                raise WorktreeError(f"worktree already exists: {name}")

            flat_slug = flatten_slug(name) # 3．构建路径和分支名
            wt_path = os.path.join(self.worktree_dir, flat_slug)
            branch_name = f"worktree-{flat_slug}"

            head_sha = self.read_worktree_head_sha(wt_path) # 3.5 快速恢复：纯文件系统读取，不调 git 子进程
            ...
            result = self._run_git([ # 4.执行 git worktree add
                "worktree", "add",
            ...
            perform_post_creation_setup( # 5．创建后设置（只对新建执行，快速恢复跳过)
                self.repo_root,
                wt_path,
    ...
    # 进入 worktree
    # ------------------------------------------------------------------
# 没有选择直接修改进程级的工作目录（cwd），而是将 Worktree 路径记录到会话状态中，让每次工具调用时显式获取当前工作目录
# 这种设计避免了直接 chdir 可能带来的并发问题
    async def enter(self, name: str) -> WorktreeSession:
        ...
        session = WorktreeSession( # 记录会话状态：原cwd、Worktree路径、原分支、原 HEAD
        ...
        save_worktree_session(self._clinecode_dir, session) # 持久化到·clinecode/worktree_session.json
        return session
    ...
    async def exit(
        ...
        if action == "remove" and not discard_changes:  # 变更保护
        ...
        save_worktree_session(self._clinecode_dir, None)

        if action == "remove": # 可选：删除Worktree
...
# 思路很简单：如果 Worktree 里没有未提交的修改、也没有新增的 commit，说明子 Agent 只是读了一些文件做了分析，
# 没留下什么有价值的东西，直接清掉。如果子 Agent 写了代码或做了 commit，Worktree 留着让主 Agent review。
    ...
    def restore_session(self) -> WorktreeSession | None:
        session = load_worktree_session(self._clinecode_dir)
        if session is None:
            return None
