...
def perform_post_creation_setup(
    ...
    _copy_local_configs(root, wt) # A.复制本地配置
    _setup_git_hooks(root, wt) # B.配置 Git Hooks
    _create_symlinks(root, wt, symlink_directories or [])  #C.创建符号链接
    _copy_ignored_files(root, wt) # D.复制被忽略的文件
