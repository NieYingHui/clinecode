...
def _session_path(clinecode_dir: Path) -> Path:
    return clinecode_dir / SESSION_FILENAME
...
def save_worktree_session(
    ...
    path = _session_path(clinecode_dir)
...
def load_worktree_session(clinecode_dir: Path) -> WorktreeSession | None:
    path = _session_path(clinecode_dir)
    if not path.exists():
        return None
    try:
