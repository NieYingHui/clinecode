...
from clinecode.worktree.changes import has_unpushed_commits, has_worktree_changes
...
# 清理逻辑:层层过滤：ephemeral模式→跳过当前 session →年龄检查→变更检查→未推送 commit 检查。
...
# 后台清理循环: 用asyncio.sleep做定时，简单的无限循环。调用方用asyncio.create_task启动。
