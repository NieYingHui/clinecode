...
class ConversationManager:
    ...
    def record_usage_anchor( # 锚点
        ...
        # 记录此时的真实 Token 消耗（baseline_tokens）和当前历史消息的长度（anchor_count）。
        self.anchor_count = len(self.history)
        # 保持旧字段同步，兼容仍在使用它的读取方。
        self.last_input_tokens = self.baseline_tokens

    def current_tokens(self) -> int:  # 增量估算 
        ...
        if self.baseline_tokens <= 0:
            # 如果没有锚点（冷启动或历史被压缩重置），则退化为对全部历史消息进行纯字符估算。
            return estimate_tokens(self.history)
        tail = self.history[self.anchor_count:]
        #如果存在锚点，则使用 baseline_tokens 加上锚点之后新增消息的字符估算 Token 数（estimate_tokens(tail)）
    ...
    def add_system_reminder(self, content: str) -> None: #向对话历史记录中添加一条“系统提醒”消息
        self.history.append(
            Message(
                role="user",
    ...
    def inject_environment(self, context: str) -> None: #将环境上下文信息注入到对话历史记录的起始位置
        if not self.env_injected:
            self.history.insert(0, Message(role="user", content=context))
            self.env_injected = True

    def inject_long_term_memory( #向系统注入长期记忆和指令
        ...
        pos = 1 if self.env_injected else 0 #根据env_injected的值决定插入位置（位置1或0）
        self.history.insert(pos, Message(role="user", content=wrapped))
        self.ltm_injected = True

    def replace_history(self, new_messages: list[Message]) -> None: # 当对话过长触发压缩或摘要机制时
