...
from clinecode.hooks.conditions import ConditionParseError, parse_condition
from clinecode.hooks.events import LifecycleEvent
from clinecode.hooks.models import Action, Hook
...
# 校验规则很明确：事件名必须在合法事件列表中，action 类型必须是 command/prompt/http/agent 四者之一，
# reject 只能用在pre_tool_use上，async 不能用在pre_tool_use上，
# 每种 action 类型要检查必填字段。比如 command 类型必须有command字段，http 类型必须有ur1字段。
...
def load_hooks(raw_hooks: list[dict] | None) -> list[Hook]:
    ...
    for i, entry in enumerate(raw_hooks):
        ...
        if reject and event != "pre_tool_use": # reject 只能用在pre_tool_use上，因为只有工具执行前才有「拦截」的语义
            raise HookConfigError(
        ...
        if async_exec and event == "pre_tool_use": # async 不能和pre_tool_use一起用，因为拦截必须等结果。
            raise HookConfigError(
