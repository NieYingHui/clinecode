...
from clinecode.hooks.models import Action, ActionResult, HookContext

log = logging.getLogger(__name__)


async def execute_command(action: Action, ctx: HookContext) -> ActionResult: # command：执行 shell 命令
    command = ctx.expand(action.command) # 先用ctx.expand 做模板变量替换
    try:
        proc = await asyncio.create_subprocess_shell( # 然后用asyncio.create_subprocess_shell启动子进程
            command,
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.STDOUT, # stderr合并到 stdout（sTDoUT参数），只需要处理一个输出流
        )
        try:
...
async def execute_prompt(action: Action, ctx: HookContext) -> ActionResult: # prompt：注入提示词
    message = ctx.expand(action.message)
    return ActionResult(output=message, success=True)


async def execute_http(action: Action, ctx: HookContext) -> ActionResult: # http：发HTTP请求
    ...
    if body and "Content-Type" not in headers:
        headers["Content-Type"] = "application/json" # 如果用户没设Content-Type，有body 时自动加application/json
    ...
    def _do_request() -> ActionResult:
        try:
            ...
            with urlopen(req, timeout=30) as resp:
                resp_body = resp.read().decode(errors="replace")[:500] # 响应体截断到 500 字符，防止超大响应撑爆内存
                return ActionResult(
    ...
    return await loop.run_in_executor(None, _do_request) # 用run_in_executor丢到线程池里执行，不会阻塞事件循环


async def execute_agent(action: Action, ctx: HookContext) -> ActionResult: # agent：启动子 Agent
