...
from clinecode.commands import (
    CommandContext,
    CommandRegistry,
    complete,
    parse_command,
)
from clinecode.commands.completion import CompletionPopup
from clinecode.commands.handlers import register_all_commands
from clinecode.config import MCPServerConfig, ProviderConfig
from clinecode.hooks import HookContext, HookEngine, load_hooks
from clinecode.conversation import ConversationManager, Message
from clinecode.mcp import get_mcp_manager as MCPManager
from clinecode.memory import (
    MemoryManager,
    Session,
    SessionManager,
...
from clinecode.permissions import (
...
from clinecode.agents.task_manager import TaskManager
from clinecode.agents.trace import TraceManager
from clinecode.agents.notification import inject_task_notifications
from clinecode.commands.handlers.tasks import create_tasks_command
from clinecode.skills.executor import SkillExecutor
from clinecode.skills.loader import SkillLoader
from clinecode.commands.handlers.skill_register import register_skill_commands
from rich.text import Text as RichText
from textual.theme import Theme
from clinecode.cache import FileCache
from clinecode.tools import ToolRegistry, create_default_registry
from clinecode.tools.agent_tool import AgentTool
from clinecode.tools.ask_user import AskUserEvent, AskUserTool
from clinecode.tools.impl.tool_search import ToolSearchTool
from clinecode.tools.load_skill import LoadSkill
from clinecode.worktree.cleanup import start_stale_cleanup_task
from clinecode.worktree.manager import WorktreeManager
from clinecode.commands.handlers.worktree import create_worktree_command
from clinecode.teammate_tree import TeammateTree
...
_SKIP_DIRS = {".git", "node_modules", ".venv", "__pycache__", ".clinecode", "build", ".gradle"}
...
class ChatInput(TextArea):
    ...
    def load_history(self, work_dir: str) -> None:
        self._history_file = Path(work_dir) / ".clinecode" / "history"
...
class ClineCodeApp(App):
    ...
    def on_mount(self) -> None:
        self.register_theme(_CLINECODE_THEME)
        self.theme = "clinecode"
        if len(self.providers) == 1:
    ...
    def _select_provider(self, provider: ProviderConfig) -> None:
        ...
        checker = PermissionChecker(
            ...
            rule_engine=RuleEngine(
                user_rules_path=home / ".clinecode" / "permissions.yaml",
                project_rules_path=Path(work_dir) / ".clinecode" / "permissions.yaml",
                local_rules_path=Path(work_dir) / ".clinecode" / "permissions.local.yaml",
            ),
        ...
        from clinecode.filehistory import FileHistory
        ...
        from clinecode.tools.exit_plan_mode import ExitPlanModeTool
        ...
        # --- Worktree 系统初始化 ---
        from clinecode.config import WorktreeConfig
        ...
        from clinecode.tools.enter_worktree import EnterWorktreeTool
        from clinecode.tools.exit_worktree import ExitWorktreeTool
        ...
        # --- Agent 团队系统初始化 ---
        from clinecode.teams.manager import TeamManager
        from clinecode.tools.team_create import TeamCreateTool
        from clinecode.tools.team_delete import TeamDeleteTool
        ...
        from clinecode.commands.handlers.trace import create_trace_command
        ...
        from clinecode.tools.synthetic_output import SyntheticOutputTool
    ...
    async def _prefetch_relevant_memories(self, query: str) -> str:
        ...
        async def selector(system_prompt: str, user_message: str) -> str:
            from clinecode.tools.base import StreamEnd, TextDelta
    ...
    async def _show_plan_approval(self) -> None:
        from clinecode.plan_dialog import InlinePlanWidget
    ...
    def on_inline_plan_widget_responded(
        ...
        from clinecode.plan_dialog import InlinePlanWidget, PlanChoice
    ...
    async def _handle_askuser(self, event: AskUserEvent) -> None:
        from clinecode.askuser_dialog import InlineAskUserWidget
    ...
    async def _handle_permission_request(self, request: PermissionRequest) -> None:
        from clinecode.permission_dialog import InlinePermissionWidget
