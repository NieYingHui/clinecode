...
# 全局禁用的 7 个工具是：Taskoutput、ExitPlanMode、EnterPlanMode、
# Agent、AskUserQuestion、TaskStop、Workflow。禁Agent是为了防止子 Agent 无限嵌套。
...
ASYNC_AGENT_ALLOWED_TOOLS: frozenset[str] = frozenset({ # 第 3 层是后台任务白名单
    "ReadFile",
    "WebSearch",
    "TodoWrite",
    "Grep",
...
IN_PROCESS_TEAMMATE_ALLOWED_TOOLS: frozenset[str] = ( # 在异步白名单基础上加了协调工具和 Cron 工具。
...
def build_teammate_tools(
    ...
    from clinecode.tools.send_message import SendMessageTool
    from clinecode.tools.task_create import TaskCreateTool
    from clinecode.tools.task_get import TaskGetTool
    from clinecode.tools.task_list import TaskListTool
    from clinecode.tools.task_update import TaskUpdateTool
