...
class TaskManager:
    ...
    def __init__(self) -> None:
        ...
        self._notify_queue: asyncio.Queue[str] = asyncio.Queue() #  asyncio.Queue()本身是协程安全的，不需要额外加锁
...
# _run_background是后台协程主体，完成后把 token 统计写入progress，再通过_notify_queue.put 推送通知。
    async def _run_background(
        ...
        try:
            ...
            if bg.agent.team_name and bg.agent._team_manager:
                ...
                if mailbox:
                    from clinecode.teams.mailbox import create_message
                    msg = create_message(
                        from_agent=bg.name,
                        to_agent="lead",
...
# adopt_running方法处理前台到后台的切換，把已有的 Agent 实例包装成BackgroundTask，启动新的 asyncio task 继续执行。
    def adopt_running(
        self,
        agent: Agent,
