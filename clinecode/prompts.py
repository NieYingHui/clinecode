...
@dataclass
class PromptSection: #章节
    name: str
    priority: int #排序权重 (数字越小越靠前)
    content: str #承载文本
...
class PromptBuilder:
    ...
    def build(self) -> str:
        ...
        parts = [s.content.strip() for s in self._sections if s.content.strip()]#strip去掉首尾空白，条件过滤空内容
        return "\n\n".join(parts)
...
# ---------------------------------------------------------------------------
# priority的间隔设成 10，方便后续在两个段落之间插入新内容，不用改已有的编号。

IDENTITY_SECTION = PromptSection( #身份信息部分
    name="Identity",
    priority=0,
    content=(
        "You are ClineCode, an AI programming assistant running in the terminal. "
...
SYSTEM_SECTION = PromptSection( #系统
    name="System",
    priority=10,
    content="""\
# System
...
DOING_TASKS_SECTION = PromptSection( #完成任务
    name="DoingTasks",
    priority=20,
    content="""\
# Doing tasks
...
EXECUTING_ACTIONS_SECTION = PromptSection( #执行操作
...
USING_TOOLS_SECTION = PromptSection( #使用工具
    name="UsingTools",
    priority=40,
    content="""\
# Using your tools
...
TONE_STYLE_SECTION = PromptSection( #语气与风格
    name="ToneStyle",
    priority=50,
    content="""\
# Tone and style
...
TEXT_OUTPUT_SECTION = PromptSection( #文本输出
    name="TextOutput",
    priority=60,
    content="""\
...
def build_system_prompt(
    ...
    coordinator_mode: bool = False, #协调员模式
    agent_catalog: list[tuple[str, str]] | None = None, #代理目录
    custom_instructions: str = "", #自定义说明 对应项目CLAUDE.md内容
    skill_section: str = "",
    memory_section: str = "",
    work_dir: str = ".",
) -> str:
    if coordinator_mode:#coordinator_mode为 True 时直接短路返回，走完全不同的 prompt 构建路径，不经过下面的七段式组装。
        from clinecode.teams.coordinator import get_coordinator_system_prompt
        return get_coordinator_system_prompt(agent_catalog=agent_catalog)
# import写在函数体内而不是文件顶部，是为了避免循环导入。teams.coordinator可能反过来依赖
# prompts模块的某些类型，顶层互相 import 就会报ImportError。延迟导入是处理循环依赖的标准手法。
    b = PromptBuilder()
...
# 排序逻辑是：项目指令→技能描述→记忆。记忆排最后因为它最接近对话上下文，放在 System Prompt尾部利用了近因效应。
    if skill_section:
...
def build_environment_context( #收集和格式化当前环境的上下文信息，生成一个结构化的文本字符串
    work_dir: str,
    active_skills: dict[str, str] | None = None, #活跃技能名称
    skill_catalog: str = "", #技能目录
    agent_catalog: str = "", #代理目录
) -> str:
    parts = [
