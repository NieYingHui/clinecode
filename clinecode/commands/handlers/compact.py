...
from clinecode.commands.registry import Command, CommandContext, CommandType
...
async def handle_compact(ctx: CommandContext) -> None:
    ...
    input_tokens, _ = ctx.ui.get_token_count() #先检查 token 数量。低于 5000 时直接返回，避免无效的 LLM 调用。
    if input_tokens < 5000:
        ctx.ui.add_system_message(f"当前 token 数 {input_tokens:,}，无需压缩")
        return

    from clinecode.agent import CompactNotification, ErrorEvent
    ...
    if isinstance(result, CompactNotification):
        ...
        if ctx.session is not None and result.boundary is not None:
            from clinecode.memory.session import make_compact_boundary
# 压缩成功后还会把 compact boundary 持久化到会话记录里，这样/session resume恢复会话时能重建压缩后的状态。
