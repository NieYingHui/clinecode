...
from clinecode.conversation import ConversationManager, ToolResultBlock, ToolUseBlock
from clinecode.conversation import ThinkingBlock as ConvThinkingBlock
from clinecode.memory.auto_memory import MemoryManager
from clinecode.permissions import (
    Decision,
    PermissionChecker,
    PermissionMode,
)
from clinecode.hooks import HookContext, HookEngine, ToolRejectedError
from clinecode.hooks.engine import HookNotification
from clinecode.prompts import build_environment_context, build_plan_mode_reminder, build_system_prompt
...
class Agent:
    ...
    def _transcript_path(self) -> str:
        if self.session_id:
            return str(Path(self.work_dir) / ".clinecode" / "sessions" / f"{self.session_id}.jsonl")
        return ""
    ...
    def _get_plan_path(self) -> Path:
        ...
        plans_dir = Path(self.work_dir) / ".clinecode" / "plans"
    ...
    async def _execute_tool(
        ...
        if self.permission_checker:
            ...
            if decision.effect == "ask":
                ...
                response = await future #ask 通过asyncio.Future 做同步
                ...
                if response == PermissionResponse.ALLOW_ALWAYS:
                    from clinecode.permissions.rules import Rule, extract_content
    ...
    def _maybe_persist_or_truncate(self, tool_use_id: str, text: str) -> str:
        from clinecode.context.manager import (
