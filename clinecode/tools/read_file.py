...
class Params(BaseModel):
    ...
    offset: int = Field(default=0, description="Line offset to start reading from (0-based)") #开始读取的行偏移量
    limit: int = Field(default=2000, description="Maximum number of lines to read") #最大读取行数，默认为2000。


class ReadFile(Tool): #用于读取文件内容并返回带有行号的结果
    name = "ReadFile"
