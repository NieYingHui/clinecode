...
from clinecode.tools.base import Tool, ToolResult
...
if TYPE_CHECKING:
    ...
    from clinecode.client import LLMClient
...
class AgentTool(Tool):
    ...
    async def execute(self, params: BaseModel) -> ToolResult:
        ...
        if p.team_name: #路径1：team name 存在→走teammate路径
        ...
        if isolation == "worktree": #路径2：定义里有worktree 隔离→走worktree路径
            return await self._execute_with_worktree(p)

        from clinecode.agents.fork import ForkError, build_forked_messages
        from clinecode.agents.parser import AgentDef
        from clinecode.agents.tool_filter import resolve_agent_tools
        from clinecode.agent import Agent as AgentClass
        ...
        if p.subagent_type is None: #路径 3：subagent type 为空→走 Fork
            from clinecode.context import clone_replacement_state
        ...
        # 前台同步执行 #路径 4：subagent_type 存在→走定义式 (sync/async)
        try:
            if is_fork:
    ...
    async def _execute_as_teammate(self, p: AgentToolParams) -> ToolResult:
        ...
        from clinecode.agents.fork import ForkError, build_forked_messages
        from clinecode.agents.parser import AgentDef
        from clinecode.agents.tool_filter import build_teammate_tools
        from clinecode.agent import Agent as AgentClass
        from clinecode.conversation import ConversationManager
        from clinecode.permissions import (
            DangerousCommandDetector,
        ...
        from clinecode.teams.models import BackendType, TeammateInfo
    ...
    def _spawn_pane_teammate(
        ...
        try:
            if backend == BackendType.TMUX:
                from clinecode.teams.spawn_tmux import spawn_tmux_teammate
                ...
                from clinecode.teams.spawn_iterm2 import spawn_iterm2_teammate
    ...
    def _select_llm(
        ...
        model_override = params.model or (
            definition.model if definition.model != "inherit" else None # 调用参数级
        )

        if model_override and model_override != "inherit":
            client = self._create_client_for_model(model_override) # 定义级
            if client is not None:
                return client

        return self._parent_agent.client # 定义级

# 流程：创建worktree→构建 worktree上下文通知→设置工作目录为worktree 路径→同步执行子 Agent →自动清理。
    async def _execute_with_worktree(self, p: AgentToolParams) -> ToolResult:
        ...
        from clinecode.worktree.integration import (
    ...
    def _create_client_for_model(self, model_alias: str) -> LLMClient | None:
        ...
        from clinecode.client import create_client
        from clinecode.config import ProviderConfig
