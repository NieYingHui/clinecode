...
import asyncio #异步执行 shell 命令
import re
import shlex #用于安全地分割 shell 命令
...
def _extract_last_command_name(command: str) -> str | None:
    ...
    # 也要处理 sudo/env 等包装命令
    try: #shlex.split 尝试安全分割该段（处理引号、转义）
...
class Bash(Tool):
    ...
    async def execute(self, params: Params) -> ToolResult:
        ...
        try: #创建子进程
            ...
            # 等待执行（带超时）communicate() 等待进程结束并获取输出。 wait_for 添加超时限制。
            stdout, stderr = await asyncio.wait_for(proc.communicate(), timeout=timeout)
        except asyncio.TimeoutError: #杀死进程，等待它结束，返回错误结果。
            proc.kill()
            await proc.wait()
