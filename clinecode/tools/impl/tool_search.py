...
class ToolSearchTool(Tool):
    ...
    async def execute(self, params: BaseModel) -> ToolResult:
        ...
        if query.startswith("select:"):
            ...
            schemas = self._registry.find_deferred_by_names(names, self._protocol) #按名称查找延迟工具
        else:
            schemas = self._registry.search_deferred( #搜索延迟工具
                query, max_results, self._protocol
            )

        if not schemas:
            deferred_names = self._registry.get_deferred_tool_names() #获取延迟工具名称
            return ToolResult(
                output=(
        ...
        for s in schemas:
            if "name" in s:
                self._registry.mark_discovered(s["name"]) #标记工具为已发现
