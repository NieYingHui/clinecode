...
from clinecode.conversation import (
    ConversationManager,
    Message,
    ToolResultBlock,
    estimate_tokens,
)
from clinecode.serialization import build_messages

# ---------------------------------------------------------------------------
# 常量
# ---------------------------------------------------------------------------

SINGLE_RESULT_CHAR_LIMIT = 50_000 # 单条工具結果的存盘阈值
AGGREGATE_CHAR_LIMIT = 200_000 # 聚合结果存盘阈值
PREVIEW_CHARS = 2_000 # 预览字符

KEEP_RECENT_TURNS = 10 # 保护窗口
OLD_RESULT_SNIP_CHARS = 2_000 # 保留的旧结果长度
SNIPPED_TAG = "<snipped>"
# 阈值公式一样：有效窗口= 上下文窗口- 20000，自动触发线= 有效窗口-13000。尾部保留预算 10000tokens/5条消息／上限40000。
SUMMARY_OUTPUT_RESERVE = 20_000 # 摘要输出
...
SESSION_SUBDIR = ".clinecode/session/tool-results"
...
class ContentReplacementState:
    seen_ids: set[str] = field(default_factory=set) #是所有见过的 tool _use_id 集合
    replacements: dict[str, str] = field(default_factory=dict) #被替換的id 到 preview字符串的映射


@dataclass
class ContentReplacementRecord: #内容替换记录
    tool_use_id: str
    replacement: str
...
# clone_replacement_state 做浅拷贝：set(src.seen_ids）和dict(src.replacements）创建独立副本，用于fork 子Agent
    ...
    for msg in conversation.history:
        ...
        for tr in fresh:
            if len(tr.content) > SINGLE_RESULT_CHAR_LIMIT:
                fp = persist_tool_result(tr.tool_use_id, tr.content, session_dir) #存盘
                preview = make_persisted_preview(tr.content, fp) #存盘后生成预览
        ...
        if total > AGGREGATE_CHAR_LIMIT: #超过 200000 就按大小降序排，从最大的开始存盘。
...
def compute_compact_threshold(context_window: int, manual: bool = False) -> int: #用于计算紧凑阈值
    effective = context_window - SUMMARY_OUTPUT_RESERVE #输出摘要保留的空间
...
def extract_summary(llm_output: str) -> str: #提取特定格式的摘要内容
...
def _compute_keep_start_index(messages: list[Message]) -> int:
    ...
    for i in range(n - 1, -1, -1): # 从尾部往回走，累积 token 数。满足 10000 tokens 或5 条消息任一条件就停，上限 40000 tokens
    ...
    return _align_keep_start_to_tool_pair(messages, keep_start) # 把配对保护逻辑抽成了独立函数_align_keep_start_to_tool_pair
...
async def auto_compact(
    ...
    for attempt in range(max_retries):
        try:
            from clinecode.tools.base import StreamEnd, StreamEvent, TextDelta
