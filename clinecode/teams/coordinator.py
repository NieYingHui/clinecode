...
# 双重锁定：feature flag 参数 +CLINECODE_CoORDINATOR_MoDE 环境变量，两个都满足才激活。
def is_coordinator_mode(enable_flag: bool = False) -> bool:
    if not enable_flag:
        return False
    val = os.environ.get("CLINECODE_COORDINATOR_MODE", "").lower()
...
def match_session_mode(
    ...
    if session_is_coordinator:
        os.environ["CLINECODE_COORDINATOR_MODE"] = "1"
    else:
...
def get_coordinator_system_prompt(agent_catalog: list[tuple[str, str]] | None = None) -> str:
    ...
    return """You are ClineCode, an AI assistant that orchestrates software engineering tasks across multiple workers.
...
def get_coordinator_user_context(
    ...
    if worker_tools is None:
        from clinecode.agents.tool_filter import IN_PROCESS_TEAMMATE_ALLOWED_TOOLS
