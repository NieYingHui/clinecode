...
from clinecode.teams.mailbox import Mailbox, create_message
...
from clinecode.teams.shared_task import SharedTaskStore
from clinecode.teams.spawn_inprocess import InProcessTeammateHandle
...
class TeamManager:
    ...
    def drain_lead_mailbox(self) -> list[str]: # Lead 感知队友状态
        notes: list[str] = []
    ...
    def _kill_pane(self, pane_id: str, backend_type: str) -> None:
        try:
            if backend_type == BackendType.TMUX.value:
                from clinecode.teams.spawn_tmux import kill_pane
                kill_pane(pane_id)
