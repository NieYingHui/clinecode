...
from clinecode.config import ConfigError, load_config
from clinecode.hooks import HookConfigError, HookEngine, load_hooks
from clinecode.permissions import PermissionMode


def main() -> None:
    # 先确保 .clinecode/ 目录存在，否则下面写 debug.log 会因目录不存在而崩溃
    Path(".clinecode").mkdir(parents=True, exist_ok=True)
    logging.basicConfig(
        level=logging.INFO,
        format="%(asctime)s %(name)s %(message)s",
        filename=".clinecode/debug.log",
        filemode="w",
    )

    parser = argparse.ArgumentParser(prog="clinecode", description="ClineCode AI coding assistant")
    parser.add_argument(
        "--mode",
    ...
    from clinecode.app import ClineCodeApp
    from clinecode.driver import NoAltScreenDriver
...
async def _run_prompt(config, permission_mode, hook_engine, prompt: str) -> None:
    from clinecode.agent import Agent
    from clinecode.client import create_client, resolve_context_window
    from clinecode.conversation import ConversationManager
    from clinecode.memory.instructions import load_instructions
    ...
    from clinecode.tools import create_default_registry
