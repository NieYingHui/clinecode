...
class ProviderConfig:
    ...
    def resolve_api_key(self) -> str:
        ...
        return os.environ.get(env_var, "") #否则从环境变量中获取
...
def resolve_env_vars(value: str) -> str: #在给定的字符串中查找并替换环境变量占位符
    return _ENV_VAR_RE.sub(lambda m: os.environ.get(m.group(1), m.group(0)), value)

# 构建一个用于子进程的环境变量字典
def build_child_env(declared_env: dict[str, str] | None) -> dict[str, str]:
    env: dict[str, str] = {}
    path = os.environ.get("PATH", "") #从当前系统的环境变量（os.environ）中获取 PATH 的值
    if path:
        env["PATH"] = path
...
class MCPServerConfig:
    name: str
    command: str | None = None #有command 就走 stdio
    args: list[str] = field(default_factory=list)
    url: str | None = None #有 url 就走HTTP
    headers: dict[str, str] = field(default_factory=dict)
    env: dict[str, str] = field(default_factory=dict)


    @property
    def is_stdio(self) -> bool: #is_stdio是只读属性，封装了判断逻辑
...
def load_config(path: Path | None = None) -> AppConfig:
    ...
    home = Path.home()
    candidates = [
        home / ".clinecode" / "config.yaml",
        cwd / ".clinecode" / "config.yaml",
        cwd / ".clinecode" / "config.local.yaml",
    ]
    ...
    if merged is None:
        raise ConfigError(
            "No config file found. Expected .clinecode/config.yaml "
            "in project or ~/.clinecode/config.yaml"
        )
    return merged
