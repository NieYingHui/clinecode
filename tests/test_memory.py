...
from clinecode.memory.instructions import (
    MAX_INCLUDE_DEPTH,
    load_instructions,
    process_includes,
)
from clinecode.memory.session import (
    RecordType,
    ResumeResult,
    Session,
    SessionManager,
...
class TestLoadInstructions:
    def test_single_layer(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        clinecode_md = tmp_path / "CLINECODE.md"
        clinecode_md.write_text("project instructions", encoding="utf-8")
    ...
    def test_multi_layer_priority(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        root_md = tmp_path / "CLINECODE.md"
        root_md.write_text("root level", encoding="utf-8")
        dotdir = tmp_path / ".clinecode"
        dotdir.mkdir()
        dot_md = dotdir / "CLINECODE.md"
...
class TestSession:
    def test_append_writes_jsonl_and_updates_meta(self, tmp_path: Path) -> None:
        sessions_dir = tmp_path / ".clinecode" / "sessions"
...
class TestMemoryManager:
    ...
    def test_load_merges_user_and_project(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        user_mem = fake_home / ".clinecode" / "memories.md"
        user_mem.parent.mkdir(parents=True)
        user_mem.write_text("### 用户偏好\n- prefer spaces", encoding="utf-8")

        project_mem = tmp_path / "project" / ".clinecode" / "memories.md"
...
class TestConversationInjection:
    def test_inject_long_term_memory(self) -> None:
        ...
        assert "clinecodeMd" in conv.history[1].content
    ...
    def test_inject_instructions_only(self) -> None:
        ...
        assert "clinecodeMd" in conv.history[0].content
...
class TestMemoryExtraction:
    def test_extraction_prompt_contains_categories(self, tmp_path: Path) -> None:
        from clinecode.memory.auto_memory import MEMORY_EXTRACTION_PROMPT
